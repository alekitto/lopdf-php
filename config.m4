...
cargo_build:
	...
	if [[ -f ./target/$CARGO_MODE_DIR/liblopdf.dylib ]] ; then \\
		cp ./target/$CARGO_MODE_DIR/liblopdf.dylib ./modules/lopdf.dylib ; fi
	if [[ -f ./target/$CARGO_MODE_DIR/liblopdf.so ]] ; then \\
		cp ./target/$CARGO_MODE_DIR/liblopdf.so ./modules/lopdf.so ; fi
